...
func _physics_process(delta):
	...
	if min_area != null and self.position.angle_to_point(min_area) > -1.56 and self.rotation > -1.57:
		self.rotation += (self.position.angle_to_point(min_area) - self.rotation) * delta * GameManager.speed * 3
	elif min_area != null and self.position.angle_to_point(min_area) < -1.56 and self.rotation > 0:
		self.rotation += (abs(self.position.angle_to_point(min_area) - (self.rotation - (PI * 2)))) * delta * GameManager.speed * 3
	elif min_area != null and self.position.angle_to_point(min_area) < -1.56 and self.rotation <= 0:
		self.rotation += (self.position.angle_to_point(min_area) - (self.rotation)) * delta * GameManager.speed * 3
	elif min_area != null and self.position.angle_to_point(min_area) > -1.56 and self.rotation <= -1.57:
		self.rotation = self.rotation + (2 * PI)
	self.position += GameManager.speed * bullet_speed * delta * Vector2(cos(self.rotation), sin(self.rotation))
	
	if hit > 1:
		var particleee = load("res://night/scenes/particles_square.tscn").instantiate()
