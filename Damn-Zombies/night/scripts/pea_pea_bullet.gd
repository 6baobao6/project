...
func _physics_process(delta):
	if !hit:
		self.position.x += GameManager.speed * bullet_speed * delta
	else:
		self.position = self.position.move_toward(direction, GameManager.speed * 1.5 * delta * self.position.distance_to(direction))
