...
func _physics_process(delta):
	...
	if health_bar.value <= 0:
		...
		particle_effect(4, 9, 12, 10, 10, Color(1, 0.88, 0.4, 1))
	elif state == 1:
		timer.wait_time = 20
		timer.start()
		last_state = state
		...
		particle_effect(7, 22, 30, 20, 10, Color(1, 0.88, 0.4, 1))
	elif state == 2:
		if last_state != 3:
			particle_effect(10, 40, 55, 30, -7, Color(1, 0.88, 0.4, 1))
		last_state = state
		sun_timer.wait_time = 30
		sun_timer.start()
		_animated_sprite.stop()
		_animated_sprite.play("idle")
		time_up.wait_time = 122
		time_up.start()
	...
	if sun_time and color_up <= 0:
		particle_effect(10, 40, 55, 30, -7, Color(1, 0.88, 0.4, 1))
		sun_time = false
		var sun_float = load("res://night/scenes/sun_float.tscn").instantiate()
		sun_float.position = self.position
		get_parent().add_child(sun_float)
	
	if sun_time:
		if self.modulate.r < 1.5:
			self.modulate += Color(0.01, 0.01, 0.01, 0) * GameManager.speed
		color_up -= 5 * GameManager.speed
	elif !sun_time and color_up < 200:
		self.modulate -= Color(0.01, 0.01, 0.01, 0) * GameManager.speed
		color_up += 5 * GameManager.speed
