...
func _physics_process(delta):
	_animated_sprite.speed_scale = 1 * GameManager.speed
	if health_bar.value <= 0:
		GameManager.danger_level += 1
		GameManager.slime_count += 1
		...
		self.queue_free()
	self.position.x -= GameManager.speed * slimespeed * delta * speed_multiplier * move
	match move:
		0:
			pass
		1:
...
func _on_animated_sprite_2d_animation_looped():
	if _animated_sprite.animation == "eat":
		for i in range(20):
			var ball = load("res://night/scenes/bad_attack_area.tscn").instantiate()
